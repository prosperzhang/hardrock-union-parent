...
@Service
public class PrimeloadLogisticsAppBootstrapService {

    private static final String APP_CODE = "PRIMELOAD-LOGISTICS";
    private static final String APP_NAME = "一车好料物流";
    private static final String DECISION_DEPT_CODE = "PRIMELOAD_LOGISTICS_DECISION_DEPT";
    private static final String DECISION_ROLE_CODE = "PRIMELOAD_LOGISTICS_DECISION_LEADER";

    private final JdbcTemplate jdbcTemplate;

    public PrimeloadLogisticsAppBootstrapService(JdbcTemplate jdbcTemplate) {
    ...
    private void ensureApp() {
        if (findAppId() == null) {
            jdbcTemplate.update("""
                ...
                VALUES (?, ?, 'LOGISTICS_PLATFORM', '/primeload-logistics', '/primeload-logistics/login', NULL, 40, 1, ?, 0)
                """, APP_CODE, APP_NAME, "一车好料物流应用");
            return;
        }
        jdbcTemplate.update("""
            UPDATE app_registry
            SET app_name = ?,
                app_type = COALESCE(app_type, 'LOGISTICS_PLATFORM'),
                home_path = COALESCE(home_path, '/primeload-logistics'),
                login_path = COALESCE(login_path, '/primeload-logistics/login'),
                description = COALESCE(description, ?),
                sort_no = CASE WHEN sort_no = 0 THEN 40 ELSE sort_no END,
                status = 1,
                deleted = 0
            WHERE app_code = ?
            """, APP_NAME, "一车好料物流应用", APP_CODE);
    }
