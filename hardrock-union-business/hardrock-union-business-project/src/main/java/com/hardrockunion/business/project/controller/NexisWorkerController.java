...
import com.hardrockunion.business.project.service.NexisWorkerService;
...
@RestController
@RequestMapping("/api/nexis/workers")
@Tag(name = "NEXIS-工人", description = "Nexis 工人管理，工人可挂在班组、施工范围和参建单位下面")
public class NexisWorkerController {

    private final NexisWorkerService workerService;

    public NexisWorkerController(NexisWorkerService workerService) {
        this.workerService = workerService;
    }

    @Operation(summary = "工人分页列表", description = "支持按项目、标段、参建单位、施工范围、班组和关键词分页查看工人。")
    @GetMapping
    public Result<PageResponse<NexisWorkerResponse>> list(NexisWorkerQueryRequest request, LoginUser loginUser) {
        return Result.success(workerService.list(request, loginUser));
    }

    @Operation(summary = "工人详情", description = "查看单个工人的项目、标段、班组、施工范围和工种信息。")
    @GetMapping("/{id}")
    public Result<NexisWorkerResponse> get(@Parameter(description = "工人 ID", example = "68270000000000002")
    ...
    @PostMapping
    public Result<NexisWorkerResponse> create(@RequestBody NexisWorkerCreateRequest request, LoginUser loginUser) {
