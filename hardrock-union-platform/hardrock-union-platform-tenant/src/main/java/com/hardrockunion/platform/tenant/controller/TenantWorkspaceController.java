...
@RestController
@Tag(name = "租户空间", description = "租户空间创建与查询。PMHUB 中 tenant 即 project，PRIMELOAD-MARKETPLACE 中 tenant 即 merchant。")
...
public class TenantWorkspaceController {
    ...
    @Operation(summary = "查询当前可见租户空间列表", description = "返回当前账号可见的租户空间列表。PMHUB 返回项目，PRIMELOAD-MARKETPLACE 返回商户。")
    @GetMapping
    ...
    @Operation(summary = "创建租户空间", description = "创建当前 app 的租户空间，并自动把创建者加入默认决策部管理员角色。")
    @PostMapping
