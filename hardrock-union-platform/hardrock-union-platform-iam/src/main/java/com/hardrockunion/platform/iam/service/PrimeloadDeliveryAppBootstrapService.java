...
@Service
public class PrimeloadDeliveryAppBootstrapService {

    private static final String APP_CODE = "PRIMELOAD-DELIVERY";
    private static final String APP_NAME = "一车好料达";
    private static final String DECISION_DEPT_CODE = "PRIMELOAD_DELIVERY_DECISION_DEPT";
    private static final String DECISION_ROLE_CODE = "PRIMELOAD_DELIVERY_DECISION_LEADER";

    private final JdbcTemplate jdbcTemplate;

    public PrimeloadDeliveryAppBootstrapService(JdbcTemplate jdbcTemplate) {
    ...
    private void ensureApp() {
        if (findAppId() == null) {
            jdbcTemplate.update("""
                ...
                VALUES (?, ?, 'DELIVERY_PLATFORM', '/primeload-delivery', '/primeload-delivery/login', NULL, 60, 1, ?, 0)
                """, APP_CODE, APP_NAME, "一车好料达应用");
            return;
        }
        jdbcTemplate.update("""
            UPDATE app_registry
            SET app_name = ?,
                app_type = COALESCE(app_type, 'DELIVERY_PLATFORM'),
                home_path = COALESCE(home_path, '/primeload-delivery'),
                login_path = COALESCE(login_path, '/primeload-delivery/login'),
                description = COALESCE(description, ?),
                sort_no = CASE WHEN sort_no = 0 THEN 60 ELSE sort_no END,
                status = 1,
                deleted = 0
            WHERE app_code = ?
            """, APP_NAME, "一车好料达应用", APP_CODE);
    }
