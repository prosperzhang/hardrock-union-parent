...
import com.hardrockunion.business.project.service.PmhubDictionaryService;
...
@RestController
@RequestMapping("/api/pmhub/dictionaries")
@Tag(name = "PMHUB-字典", description = "PMHub 前端下拉和枚举字典查询")
public class PmhubDictionaryController {

    private final PmhubDictionaryService dictionaryService;

    public PmhubDictionaryController(PmhubDictionaryService dictionaryService) {
        this.dictionaryService = dictionaryService;
    }

    @Operation(summary = "参建单位类型字典", description = "返回总包、专业分包、劳务分包、供应商等参建单位类型选项。")
    @GetMapping("/participant-company-types")
    public Result<List<PmhubDictionaryOptionResponse>> participantCompanyTypes() {
    ...
    public Result<List<PmhubDictionaryOptionResponse>> participantRoles() {
    ...
    public Result<List<PmhubDictionaryOptionResponse>> siteWorkScopeTypes() {
        return Result.success(dictionaryService.listSiteWorkScopeTypes());
    }

    @Operation(summary = "通用状态字典", description = "返回 PMHub 侧通用启停状态选项，当前 1 表示启用，0 表示停用。")
    @GetMapping("/record-statuses")
    public Result<List<PmhubDictionaryOptionResponse>> recordStatuses() {
    ...
    public Result<List<PmhubDictionaryOptionResponse>> realNameStatuses() {
    ...
    public Result<List<PmhubDictionaryOptionResponse>> workerEntryStatuses() {
    ...
    public Result<List<PmhubDictionaryOptionResponse>> workerAttendanceStatuses() {
